...
const RemindersDashboard = () => {
  ...
  const validate = () => {
    const newErrors = {};
    ["managementType", "locationName", "purpose", "date", "time"].forEach((field) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editId) {
        await axios.put(`http://localhost:5000/reminders/${editId}`, formData, config);
      } else {
  ...
  const renderPurposeOptions = () => {
    switch (formData.managementType) {
      case "Crop":
        return ["land preparation", "seed sowing", "irrigation", "fertilization", "weed monitoring", "pest management", "harvesting", "post harvesting", "others"];
      case "Livestock":
        return ["health check", "vaccination", "feeding", "milking", "cleaning", "maintenance", "others"];
      case "Resource":
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <h1 className="text-3xl font-bold text-center text-green-700 mb-6">FMS Reminders Service</h1>
        ...
        <div className="bg-white p-4 rounded shadow-md mb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <select name="managementType" value={formData.managementType} onChange={handleChange}
              ...
              {errors.managementType && <p className="text-red-500 text-sm">{errors.managementType}</p>}
            </div>

            <div>
              <label className="font-semibold">
                {formData.managementType === "Crop" ? "Plot Name" :
                  formData.managementType === "Livestock" ? "Farm Name" :
                    formData.managementType === "Resource" ? "Store Name" : "Name"}
              </label>
              <input
                type="text"
                name="locationName"
                ...
                className="w-full p-2 border rounded"
              />
              {errors.locationName && <p className="text-red-500 text-sm">{errors.locationName}</p>}
            </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white shadow-md rounded">
          ...
          <tbody>
            {reminders.length === 0 ? (
              <tr>
                <td colSpan="8" className="text-center py-4 text-gray-500">No reminders found</td>
              </tr>
            ) : (
              reminders.map((r, idx) => (
